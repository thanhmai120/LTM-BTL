...
public class PlayerHomeFrm extends javax.swing.JFrame {
    ...
    private FriendRequest frAnswering;
    private Invitation invAnswering;
...
//        mySocket.sendData(new ObjectWrapper(ObjectWrapper.GET_LIST_FRIEND, null));
        mySocket.sendData(new ObjectWrapper(ObjectWrapper.GET_LIST_PLAYER_TOURNAMENT, mySocket.getUser()));
        mySocket.sendData(new ObjectWrapper(ObjectWrapper.GET_LIST_PLAYER_RANK, mySocket.getUser()));
        mySocket.sendData(new ObjectWrapper(ObjectWrapper.GET_LIST_FRIEND_REQUEST, mySocket.getUser()));
        mySocket.sendData(new ObjectWrapper(ObjectWrapper.GET_LIST_INVITATION, mySocket.getUser()));
    }
...
//                    JOptionPane.showMessageDialog(frm, "this function is under construction");
                    ...
                    for(ObjectWrapper func: mySocket.getActiveFunction())
                        if(func.getData() instanceof TournamentFrm) {
                            ((TournamentFrm) func.getData()).dispose();
                            existed.add(func);
                        }
                    for(ObjectWrapper func : existed)
                        mySocket.getActiveFunction().remove(func);
                    new TournamentFrm(mySocket,listTournamentPlayer.get(row).getTournament(),listFriend).setVisible(true);
                }
            }
        });
        ...
        tblFriendRequest.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                int column = tblFriendRequest.getColumnModel().getColumnIndexAtX(e.getX()); // get the coloum of the button
                int row = e.getY() / tblFriendRequest.getRowHeight(); // get the row of the button
 
                // *Checking the row or column is valid or not
                if (row < tblFriendRequest.getRowCount() && row >= 0) {
                    
                    FriendRequest fr = listFriendRequest.get(row);
                    if(tblFriendRequest.getColumnName(column).equals("Accept")){
                        fr.setAccepted(true);
                        mySocket.sendData(new ObjectWrapper(ObjectWrapper.ANSWER_FRIEND_REQUEST, fr));
//                        listFriendRequest.remove(fr);
//                        updateTableFriendRequest();
                        frAnswering = fr;
                        
                    }
                    else if(tblFriendRequest.getColumnName(column).equals("Delete")){
...
//                        updateTableFriendRequest();
        ...
        tblInvitation.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                int column = tblInvitation.getColumnModel().getColumnIndexAtX(e.getX()); // get the coloum of the button
                int row = e.getY() / tblInvitation.getRowHeight(); // get the row of the button
 
                // *Checking the row or column is valid or not
                if (row < tblInvitation.getRowCount() && row >= 0) {
                    
                    Invitation i =listInvitation.get(row);
                    if(tblInvitation.getColumnName(column).equals("Accept")){
                        i.setAccepted(true);
                        mySocket.sendData(new ObjectWrapper(ObjectWrapper.ANSWER_INVITATION, i));
//                        listFriendRequest.remove(fr);
//                        updateTableFriendRequest();
                        invAnswering = i;
                        
                    }
                    else if(tblInvitation.getColumnName(column).equals("Decline")){
                        i.setAccepted(false);
                        mySocket.sendData(new ObjectWrapper(ObjectWrapper.ANSWER_INVITATION, i));
//                        listFriendRequest.remove(fr);
//                        updateTableFriendRequest();
    ...
    private void initComponents() {
        ...
        tblTournament.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Tournament", "Begin", "End", "Day join", "Invite"
            }
        ));
        jScrollPane5.setViewportView(tblTournament);

        jLabel7.setFont(new java.awt.Font("Tahoma", 0, 18)); // NOI18N
        jLabel7.setText("Join Tournament Invitations");
        ...
        tblInvitation.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Tournament", "From", "Expire time", "Accept"
            }
        ));
        jScrollPane7.setViewportView(tblInvitation);
    ...
    public void receivedDataProcessing(ObjectWrapper data) {
        if(data.getPerformative() == ObjectWrapper.SERVER_INFORM_CHALLENGE){
        ...
        else if(data.getPerformative() == ObjectWrapper.SERVER_INFORM_FRIEND_REQUEST) {
            FriendRequest fr = (FriendRequest) data.getData();
            listFriendRequest.add(fr);
            updateTableFriendRequest();
            int fromPlayerID = fr.getFromPlayer().getID();
            for(PlayerRank pr : listRank) 
                if(pr.getID() == fromPlayerID) {
                    pr.setFriend_stat(PlayerStat.REQUESTING);
                }
        }
        else if(data.getPerformative() == ObjectWrapper.REPLY_GET_LIST_FRIEND_REQUEST) {
            listFriendRequest = (ArrayList<FriendRequest>) data.getData();
            updateTableFriendRequest();
        }
        else if(data.getPerformative() == ObjectWrapper.REPLY_ANSWER_FRIEND_REQUEST) {
            if(data.getData() instanceof String && data.getData().equals("ok")) {
                JOptionPane.showMessageDialog(this, "Answer friend request sent");
                if(frAnswering != null){
                    if(listFriendRequest.contains(frAnswering)){
                        listFriendRequest.remove(frAnswering);
                    }
                    updateTableFriendRequest();
                    int fromPlayerID = frAnswering.getFromPlayer().getID();
                    for(PlayerRank pr : listRank) 
                        if(pr.getID() == fromPlayerID) {
                            if(frAnswering.isAccepted())
                                pr.setFriend_stat(PlayerStat.BE_FRIEND);
                            else
                                pr.setFriend_stat(0);
                        }
                    refreshListFriend();
            ...
            if(data.getData() instanceof String && data.getData().equals("fail")) {
                JOptionPane.showMessageDialog(this, "Answer friend request fail");
                frAnswering = null;
            }
        }
        else if(data.getPerformative() == ObjectWrapper.SERVER_INFORM_ANSWER_FRIEND_REQUEST) {
            if(data.getData() instanceof FriendRequest) {
                FriendRequest fr = (FriendRequest) data.getData();
                if(fr.isAccepted()) 
                    JOptionPane.showMessageDialog(this, fr.getToPlayer().getFullname()+" accept your request. You are friends now");
                int toPlayerID = fr.getToPlayer().getID();
                for(PlayerRank pr : listRank) 
                    if(pr.getID() == toPlayerID) {
                        if(fr.isAccepted())
                            pr.setFriend_stat(PlayerStat.BE_FRIEND);
                        else
                            pr.setFriend_stat(0);
        ...
        else if(data.getPerformative() == ObjectWrapper.SERVER_INFORM_UNFRIEND) {
            if(data.getData() instanceof FriendRequest) {
                FriendRequest fr = (FriendRequest) data.getData();
                for(PlayerRank pr : listRank) 
                    if(pr.getID() == fr.getFromPlayer().getID()) 
                        pr.setFriend_stat(0);
                refreshListFriend();
            }
        }
        else if(data.getPerformative() == ObjectWrapper.REPLY_GET_LIST_INVITATION) {
            if(data.getData() instanceof ArrayList) {
                listInvitation = (ArrayList<Invitation>) data.getData();
                updateTableInvitation();
            }
        }
        else if(data.getPerformative() == ObjectWrapper.SERVER_INFORM_INVITATION) {
            if(data.getData() instanceof Invitation) {
                listInvitation.add((Invitation)data.getData());
                updateTableInvitation();
            }
        }
        else if(data.getPerformative() == ObjectWrapper.REPLY_ANSWER_INVITATION) {
            if(data.getData() instanceof String && data.getData().equals("ok")) {
                JOptionPane.showMessageDialog(this, "Answer invitation sent");
                if(invAnswering != null){
                    if(listInvitation.contains(invAnswering)){
                        listInvitation.remove(invAnswering);
                    }
                    updateTableInvitation();
                    invAnswering = null;
                }
            ...
            if(data.getData() instanceof String && data.getData().equals("fail")) {
                JOptionPane.showMessageDialog(this, "Answer invitation fail");
                frAnswering = null;
            }
        }
        else if(data.getPerformative() == ObjectWrapper.SERVER_INFORM_NEW_TOURNAMENT_PLAYER) {
            if(data.getData() instanceof TournamentPlayer ) {
                TournamentPlayer tp = (TournamentPlayer) data.getData();
                if(tp.getID()>0 && tp.getPlayer().getID() == mySocket.getUser().getID()) {
    ...
    public void updateTableInvitation(){
        String[] columnNames = {"Tournament","From","Expire","Accept","Decline"};
        Object[][] value = new Object[listInvitation.size()][columnNames.length];
        for(int i=0; i<listInvitation.size(); i++){
            value[i][0] = listInvitation.get(i).getTournament().getTrm_name();
            value[i][1] = listInvitation.get(i).getUser().getFullname()
                    +" ("+listInvitation.get(i).getUser().getType()+")";
            value[i][2] = (listInvitation.get(i).getExpired() == null ? "none" : listInvitation.get(i).getExpired().toString());
            value[i][3] = new JButton("accept");
            value[i][4] = new JButton("decline");
        }
        JTableButtonModel model = new JTableButtonModel();
        model.setColumns(columnNames);
        model.setRows(value);
        tblInvitation.setModel(model);
        TableCellRenderer tableRenderer = tblInvitation.getDefaultRenderer(JButton.class);
        tblInvitation.setDefaultRenderer(JButton.class, new JTableButtonRenderer(tableRenderer));
    }
    
    public void refreshListFriend() {
        listFriend = new ArrayList<PlayerRank>();
        for(PlayerRank r: listRank){
    ...
    public void updateTableFriendRequest(){
        ...
        Object[][] value = new Object[listFriendRequest.size()][columnNames.length];
        for(int i=0; i<listFriendRequest.size(); i++){
            ...
            value[i][2] = new JButton("decline");
        }
        JTableButtonModel model = new JTableButtonModel();
        model.setColumns(columnNames);
        model.setRows(value);
        tblFriendRequest.setModel(model);
        TableCellRenderer tableRenderer = tblFriendRequest.getDefaultRenderer(JButton.class);
